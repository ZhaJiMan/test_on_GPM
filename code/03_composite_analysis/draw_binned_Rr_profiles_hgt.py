#----------------------------------------------------------------------------
# 2021/10/15
# 同draw_binned_LH_profiles.py,但垂直坐标为高度.
#----------------------------------------------------------------------------
import json
...
def calc_profiles(ds1, ds2, bins, xvarname, yvarname):
    ...
    返回高度坐标,廓线条数,平均廓线和标准误差廓线.
    '''
    nbin = len(bins) - 1
    height = ds1.height.data
    # 提前准备好数组.
    npoints = np.zeros((2, nbin), dtype=int)
    mean_profiles = np.ma.masked_all((2, nbin, len(height)))
    sem_profiles = np.ma.masked_all((2, nbin, len(height)))
    # 对两组Dataset进行循环.
    ...
    # 去掉2km高度以下的数据.
    mean_profiles = mean_profiles[:, :, height >= 2]
    sem_profiles = sem_profiles[:, :, height >= 2]
    ...
    # 用namedtuple存储结果.
    Data = namedtuple(
        'Data', ['height', 'npoints', 'mean_profiles', 'sem_profiles']
    )

    return Data(height, npoints, mean_profiles, sem_profiles)
...
if __name__ == '__main__':
    ...
    draw_Rr_profiles(
        ...
        output_filepath=output_dirpath / 'Rr_profiles_hgt.png'
    )
