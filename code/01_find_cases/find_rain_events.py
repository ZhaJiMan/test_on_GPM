#-----------------------------------------------------------------------------
# 2021/05/08
# 从GPM DPR文件中找出华东地区发生的降水事件.
#
# 时间范围,空间范围,以及降水像元的阈值数目都由配置文件config.json给出.
# 程序中只选取了春季(3~5月)的数据,根据需求不同可以在程序中修改.
#
# 寻找降水事件的算法为:依次读取每个DPR文件,判断是否有像元落入DPR_extent中,
# 接着判断落入其中的有降水的像元数量是否大于给定的阈值.若这些都满足,则记为
# 一次华东的降水事件.
#
# 程序最后会把找出的降水事件记录到rain_events.json文件中.
# 会记录对应的DPR,ENV和SLH文件路径,发生降水的大致时间和经纬度.
#-----------------------------------------------------------------------------
import json
...
if __name__ == '__main__':
    ...
    for date in dates:
        for DPR_filepath in get_DPR_filepath(date):
            # 结果通过回调函数收集,不能保证结果有序.
            p.apply_async(
                process_one_file, args=(DPR_filepath,),
                callback=collect_results
            )
    p.close()
    p.join()
    # 根据降水时间,对结果进行排序.
    results.sort(key=lambda x: x['rain_time'])

    # 把找到的结果写到json文件中.
    output_filepath = Path(config['result_dirpath']) / 'rain_events.json'
