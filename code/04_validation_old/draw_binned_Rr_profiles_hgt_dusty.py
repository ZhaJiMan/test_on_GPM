...
def calc_profiles(ds1, ds2, bins, xvarname, yvarname, smooth=False):
    ...
    # 去掉10℃高度以下的数据.
    mean_profiles = mean_profiles[:, :, hgt >= 1.5]
    sem_profiles = sem_profiles[:, :, hgt >= 1.5]
    hgt = hgt[hgt >= 1.5]
...
if __name__ == '__main__':
    ...
    dusty_ds = xr.load_dataset(str(input_dirpath / 'dusty_profile.nc'))
    clean_ds = xr.load_dataset(str(input_dirpath / 'clean_profile.nc'))

    # 若输出目录不存在,那么新建.
    output_dirpath = Path(config['result_dirpath']) / 'Rr_profiles_hgt_dusty'
