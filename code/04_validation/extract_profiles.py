#----------------------------------------------------------------------------
# 2021/10/15
# 类似于02_extract_profiles/extract_profiles.py
# 但选取2014-2020年间所有的陆面降水廓线,并连接为一整个文件.
#
# 其它区别:
# - 不是按降水系统划分,而是选取研究范围内所有数据.
# - 没用到GMI和ERA5数据.
# - 记录了廓线的月份.
#----------------------------------------------------------------------------
import json
...
def concat_files(input_dirpath, output_dirpath):
    '''将每个nc文件沿npoint维连接起来,再保存为一个nc文件.'''
    ...
    output_filepath = output_dirpath / 'all_profile.nc'
...
if __name__ == '__main__':
    ...
    merged_dirpath = Path(config['temp_dirpath']) / 'merged_new'
    ...
    # 合并每个文件的结果.
    concat_files(single_dirpath, merged_dirpath)
